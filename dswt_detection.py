...
def plot_cycling_map_with_transects(roms_ds:xr.Dataset, transects_file=None, l_dswt=None,
                                    parameter='temp', vmin=20, vmax=22, cmap='RdYlBu_r',
                                    t_interval=1, transect_interval=10,
                                    lon_range=None, lat_range=None) -> plt.axes:
    
    if parameter not in roms_ds.variables:
        raise ValueError(f'Unknown parameter requested: {parameter}')
    ...
    if transects_file is not None:
        ...
        for i in np.arange(0, len(transect_names), transect_interval):
            eta, xi = get_eta_xi_along_transect(roms_ds.lon_rho.values, roms_ds.lat_rho.values,
                                                all_transects[transect_names[i]]['lon_land'],
                                                all_transects[transect_names[i]]['lat_land'],
                                                all_transects[transect_names[i]]['lon_ocean'],
                                                all_transects[transect_names[i]]['lat_ocean'],
                                                500.)
            transects_lons.append(roms_ds.lon_rho.values[eta, xi])
            transects_lats.append(roms_ds.lat_rho.values[eta, xi])
    ...
    def single_plot(fig, req_time):
        ...
        c1 = roms_ds[parameter][t, -1, :, :].plot(ax=ax1, transform=ccrs.PlateCarree(), x='lon_rho', y='lat_rho',
                                             vmin=vmin, vmax=vmax, cmap=cmap, add_colorbar=False)
        ax1.set_title('Surface')
        
        # --- Bottom
        ax2 = plt.subplot(1, 2, 2, projection=ccrs.PlateCarree())
        c2 = roms_ds[parameter][t, 0, :, :].plot(ax=ax2, transform=ccrs.PlateCarree(), x='lon_rho', y='lat_rho',
                                            vmin=vmin, vmax=vmax, cmap=cmap, add_colorbar=False)
        ...
        cbax = fig.add_axes([l2+w2+0.02, b2, 0.02, h2])
        cbar = plt.colorbar(c2, cax=cbax)
        
        ax1.set_position([l1, b1, w2, h2])
        ax2.set_position([l2, b2, w2, h2])
        
        # --- Transects on both plots
        if l_dswt is not None:
            for i, ii in enumerate(np.arange(0, len(transect_names), transect_interval)):
                if l_dswt[t, ii].astype(bool) == True:
                    ax1.plot(transects_lons[i], transects_lats[i], '-k')
                    ax2.plot(transects_lons[i], transects_lats[i], '-k')
                else:
                    ax1.plot(transects_lons[i], transects_lats[i], '-', color='#989898')
                    ax2.plot(transects_lons[i], transects_lats[i], '-', color='#989898')
            legend_elements = [Line2D([0], [0], color='k'), Line2D([0], [0], color='#989898')]
            ax2.legend(legend_elements, ['DSWT', 'no DSWT'], loc='upper left')
            
        else:
            for i in range(len(transects_lons)):
                ax1.plot(transects_lons[i], transects_lats[i], '-k')
                ax2.plot(transects_lons[i], transects_lats[i], '-k')
            legend_elements = [Line2D([0], [0], '-k')]
            ax2.legend(legend_elements, ['Transects'], loc='upper left')
        ...
        title = f'{roms_ds.ocean_time[t].values}'
        plt.suptitle(title)
...
if __name__ == '__main__':
    ...
    transects_file = 'input/transects_cwa_114-116E_33-31S.json'

    l_dswt = determine_dswt_along_multiple_transects(roms_ds, transects_file)
    
    plot_cycling_map_with_transects(roms_ds, transects_file=transects_file, l_dswt=l_dswt,
                                    lon_range=[114.0, 116.0], lat_range=[-33.0, -31.0], transect_interval=5)
