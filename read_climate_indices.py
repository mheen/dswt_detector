...
def read_mei_data(input_path=get_dir_from_json('mei'), skiprows=2, year_range=None) -> tuple[np.ndarray[datetime], np.ndarray[float]]:
    ...
    if year_range is not None:
        l_time = get_l_time_range(time, datetime(year_range[0], 1, 1), datetime(year_range[1], 12, 31))
        time = time[l_time]
        mei = mei[l_time]
...
def read_dmi_data(input_path=get_dir_from_json('dmi'), year_range=None) -> tuple[np.ndarray[datetime], np.ndarray[float]]:
    ...
    time_monthly_mean = np.array(time_monthly_mean)
    dmi_monthly_mean = np.array(dmi_monthly_mean)

    if year_range is not None:
        l_time = get_l_time_range(time_monthly_mean, datetime(year_range[0], 1, 1), datetime(year_range[1], 12, 31))
        time_monthly_mean = time_monthly_mean[l_time]
