...
def plot_glider_model_dswt(glider_ds:xr.Dataset, model_ds:xr.Dataset, config:Config,
                           output_path=None, show=False, flip_plot=False,
    ...
    max_dist = np.nanmax(glider_distance)
    ...
    if flip_plot == True:
        subtitle_loc = 'lower right'
    else:
        subtitle_loc = 'lower left'
    
    fig = plt.figure(figsize=(10, 5))    
    
    def _plot_glider_transect(ax, values, vmin, vmax, cmap):
        if flip_plot == True:
            c = ax.pcolormesh(glider_distance, glider_z, np.flip(values, axis=1), cmap=cmap, vmin=vmin, vmax=vmax)
            ax.fill_between(glider_distance, glider_z[0], np.flip(z_bottom, axis=0), color='#d2d2d2', edgecolor='k')
            xticks = np.arange(max_dist, 0, -10000)
        else:
            c = ax.pcolormesh(glider_distance, glider_z, values, cmap=cmap, vmin=vmin, vmax=vmax)
            ax.fill_between(glider_distance, glider_z[0], z_bottom, color='#d2d2d2', edgecolor='k')
    ...
    if flip_plot == True:
        ax1.scatter(max_dist-glider_dswt_x, -glider_dswt_h + glider_dswt_dz, marker='o', c='w', s=40)
        ax1.scatter(max_dist-glider_dswt_x, -glider_dswt_h + glider_dswt_dz, marker='o', c='k', s=20)
    else:
    ...
    add_subtitle(ax1, f'(a) Ocean glider', location=subtitle_loc)
    ...
    ax2.set_yticklabels([])
    ax2.set_ylabel('')
    add_subtitle(ax2, f'(b) Ocean glider', location=subtitle_loc)
    ...
    ax3.set_yticklabels([])
    ax3.set_ylabel('')
    add_subtitle(ax3, f'(c) Ocean glider', location=subtitle_loc)
    ...
    def _plot_model_transect(ax, values, vmin, vmax, cmap):
        if flip_plot == True:
            c = ax.pcolormesh(model_x, np.flip(model_z, axis=1), np.flip(values, axis=1), cmap=cmap, vmin=vmin, vmax=vmax)
            ax.fill_between(model_x, ymin_model, np.flip(model_h, axis=0), color='#d2d2d2', edgecolor='k')
            xticks = np.arange(max_dist, 0, -10000)
            xticklabels = abs(xticks-max_dist) / 1000
        else:
    ...
    add_subtitle(ax4, '(d) CWA-ROMS', location=subtitle_loc)
    ...
    ax5.set_yticklabels([])
    ax5.set_ylabel('')
    add_subtitle(ax5, '(e) CWA-ROMS', location=subtitle_loc)
    ...
    ax6.set_yticklabels([])
    ax6.set_ylabel('')
    add_subtitle(ax6, '(f) CWA-ROMS', location=subtitle_loc)
    ...
    _plot_colorbar(ax4, c1, r'$\sigma_T$ (kg m$^{-3}$)')
...
def compare_glider_model_dswt(input_files, start_dates, end_dates, config, flip, glider_dir, model_dir, model_grid_file,
                              output_dir, vmin1, vmax1, vmin2, vmax2, vmin3, vmax3, flip_plot):
...
if __name__ == '__main__':
    ...
    vmin1 = [24.8, 24.8, None, None, None, None, None, None]
    vmax1 = [25.6, 25.6, None, None, None, None, None, None]
    vmin2 = [20.0, 19.0, None, None, None, None, None, None]
    vmax2 = [22.0, 21.0, None, None, None, None, None, None]
    vmin3 = [35.4, 35.4, None, None, None, None, None, None]
    vmax3 = [35.7, 35.6, None, None, None, None, None, None]
    flip_plot = [False, False, False, False, False, False, False, False]
    ...
    compare_glider_model_dswt(input_files, start_dates, end_dates, config, flip, glider_dir, model_dir, model_grid_file, output_dir,
                              vmin1, vmax1, vmin2, vmax2, vmin3, vmax3, flip_plot)
